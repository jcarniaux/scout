"""
ZipRecruiter job crawler using JobSpy.
Triggered by Step Functions as part of the daily crawl pipeline.

ZipRecruiter does NOT use the country_indeed parameter (that is
Indeed/Glassdoor-specific). It does share the hours_old / is_remote
mutual exclusion limitation when going through JobSpy.
"""
import json
import os
import logging
...
def _build_scrape_kwargs(
    ...
    """
    Build keyword arguments for scrape_jobs().
    Avoids combining hours_old with is_remote.
    """
    kwargs: dict = {
...
def handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    """
    Crawl ZipRecruiter for jobs and send to SQS queue for enrichment.
    """
