...
def handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    for role in ROLE_QUERIES:
        for location_config in LOCATIONS:
            ...
            try:
                logger.info(f"Crawling ZipRecruiter: role={role}, location={location}, remote={is_remote}")
                ...
                logger.error(f"Error crawling ZipRecruiter for {role} in {location}: {e}", exc_info=True)
                total_errors += 1
                continue
