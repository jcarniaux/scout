"""
Glassdoor job crawler using JobSpy.
Triggered by Step Functions as part of the daily crawl pipeline.

NOTE: Glassdoor uses the same Indeed engine in JobSpy internally, so it
shares the limitation where only ONE of hours_old, job_type/is_remote,
or easy_apply can be used per scrape call.
"""
import json
import os
import logging
...
def _build_scrape_kwargs(
    ...
    """
    Build keyword arguments for scrape_jobs(), respecting the
    hours_old / is_remote mutual exclusion on Glassdoor.
    """
    kwargs: dict = {
        "site_name": ["glassdoor"],
        "search_term": role,
...
def handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    """
    Crawl Glassdoor for jobs and send to SQS queue for enrichment.
    """
